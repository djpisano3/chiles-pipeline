...
CHILES_pipe_restore.py needs to be run beforehand

'''

# This module should clear existing data from the CORRECTED and MODEL columns.
# It should also restore flags to the relevant state for restarting the pipeline
# (either totally removed or after the "initial" module).
# Finally, it will remove calibration tables.  

pipeline_scripts = ['startup', 'module1', 'bandpass']
...
for i in range( script_index, len(pipeline_scripts) ):
    try:
        execfile(pipepath+'CHILES_pipe_'+pipeline_scripts[i]+'.py')
    except Exception, e:
